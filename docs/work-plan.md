...
## Phase 5: 나머지 위젯 구현 (공통 컴포넌트 재사용) ✅ (완료)

### 5.1 기술 뉴스 헤드라인 위젯 (우선순위: 중간) ✅
- [x] **API 통합**
  - Dev.to API 연동 (`src/utils/newsApi.ts`)
  - 키워드 기반 필터링
  - 캐싱 메커니즘 (10분 TTL)
...
- [x] **API 통합**
  - Stack Exchange API 연동 (`src/utils/stackoverflowApi.ts`)
  - API 키 설정 (선택사항)
  - 태그 기반 질문 조회
  - 캐싱 메커니즘 (10분 TTL)
...
### 완료된 Phase 5 요약
- **구현 파일**:
  - `src/utils/newsApi.ts` - Dev.to API 통합
  - `src/utils/stackoverflowApi.ts` - Stack Exchange API 통합
  - `src/widgets/TechNewsWidget.tsx` - 기술 뉴스 위젯
  - `src/widgets/StackOverflowWidget.tsx` - Stack Overflow 위젯
  - `src/App.tsx` - 모든 위젯 통합
- **주요 기능**:
  - 기술 뉴스: Dev.to API를 통한 키워드 기반 뉴스 조회
  - Stack Overflow: Stack Exchange API를 통한 태그 기반 질문 조회
  - 공통 컴포넌트 재사용 (Widget, Skeleton, ErrorState, Modal)
  - 설정 관리 (키워드/태그 추가/삭제)
  - 캐싱 및 재시도 로직
- **발견된 패턴**:
  1. **API 통합 패턴**: GitHub 위젯과 동일한 패턴 (캐싱, 재시도, 에러 처리)
  2. **설정 관리 패턴**: Modal을 통한 일관된 설정 UI
  3. **상태 관리 패턴**: Zustand persist로 설정 저장
  4. **공통 컴포넌트 활용**: Widget, Skeleton, ErrorState, Modal 모두 재사용

---

## Phase 6: 대시보드 레이아웃 및 통합 ✅ (완료)

### 6.1 대시보드 레이아웃
- [x] **react-grid-layout 통합**
  - 기본 레이아웃 설정
  - 드래그 앤 드롭 기능 (헤더 영역 드래그)
  - 리사이즈 기능 (우측 하단 핸들)
...
- [x] **레이아웃 저장**
  ...
  - 세션 간 레이아웃 유지
  - breakpoint별 레이아웃 저장 (lg, md, sm, xs)

### 6.2 위젯 통합
- [x] **모든 위젯을 대시보드에 통합**
  - 기본 배치 설정 (2x2 그리드)
  - 위젯 간 일관성 유지
  - ErrorBoundary로 위젯별 독립적 에러 처리

### 6.3 반응형 디자인
- [x] **데스크톱 (lg: 1200px+)**
  - 2x2 그리드 레이아웃 (각 위젯 w:6, h:5)
  - 12열 그리드 시스템

- [x] **태블릿 (md: 996px+, sm: 768px+)**
  - 2x2 그리드 레이아웃 (md)
  - 1열 세로 배치 (sm)
...
**예상 소요 시간**: 2-3일 (실제 완료: 2025-12-05)

### 완료된 Phase 6 요약
- **구현 파일**:
  - `src/App.tsx` - ResponsiveGridLayout 통합
  - `src/stores/dashboardStore.ts` - 레이아웃 저장 및 관리
  - `src/components/Widget.tsx` - 드래그 핸들 추가
  - `src/index.css` - react-grid-layout 스타일 커스터마이징
- **주요 기능**:
  - 드래그 앤 드롭: 위젯 헤더를 드래그하여 위치 변경
  - 리사이즈: 우측 하단 핸들로 크기 조정
  - 반응형: breakpoint별 자동 레이아웃 전환
  - 레이아웃 저장: Local Storage에 자동 저장 및 복원
  - 레이아웃 초기화: 비정상 레이아웃 자동 감지 및 리셋
- **레이아웃 구조**:
  - lg/md: 2x2 그리드 (각 위젯 450px 높이)
  - sm/xs: 1열 세로 배치
  - rowHeight: 90px
  - margin: 16px
...
- [x] **ARIA 레이블**
  - 모든 대화형 요소에 레이블 추가
  - 스크린 리더 지원
  - role, aria-label, aria-labelledby 추가
...
## Phase 10: 네비게이션 및 메뉴 구조 개편 (Navigation and Menu Structure)

현재의 위젯 기반 대시보드를 **다기능 애플리케이션**으로 확장하기 위한 메뉴 및 네비게이션 구조 구현입니다.

### 10.1 핵심 네비게이션

#### 🏠 대시보드 (Dashboard)
- [ ] **현재의 커스터마이징된 메인 화면** 유지
- [ ] 위젯 배치, 드래그/드롭 기능 활성화
- [ ] **다중 레이아웃 저장 및 전환** 기능의 진입점 역할
- [ ] 레이아웃 전환 탭 또는 드롭다운 UI 구현

#### ✅ 할 일/일정 (Tasks & Calendar)
- [ ] **'오늘 할 일 위젯'의 전체 화면 뷰** 구현
- [ ] 향후 **'캘린더 통합 위젯'** 편입 준비
- [ ] 단순 목록 관리를 넘어 프로젝트 및 날짜 기반의 **심화 일정 관리** 기능
- [ ] Google Calendar API 연동 (모킹 가능)

#### ⚙️ 설정 (Settings)
- [ ] **위젯별 설정 패널** 통합 관리
  - 관심 태그, 키워드, GitHub 연동 설정
- [ ] **비기능적 설정** 통합 관리
  - 다크 모드 토글
  - 알림 설정
  - API 키 관리
...
#### 💻 활동 기록 (Activity Log)
- [ ] **'GitHub 활동 위젯'의 전체 화면 뷰** 구현
- [ ] 향후 **'프로젝트 통계 차트 위젯'** 포함
- [ ] 활동 내역의 **검색, 필터링, 기간별 분석** 기능
- [ ] 차트 라이브러리 연동 (Recharts 등)
  - 최근 30일 커밋 수 추이
  - 이슈 해결 속도 시각화

#### 📚 지식 & 트렌드 (Knowledge & Trends)
- [ ] **'기술 뉴스 위젯'과 'Stack Overflow 위젯' 통합**
- [ ] 향후 **'아이디어 메모/스크랩 위젯'** 연동
- [ ] 정보 큐레이션 및 학습 기록 기능 강화
- [ ] AI 기반 요약 기능 통합
  - GitHub 활동 요약
  - 뉴스 요약
...
- [ ] **라우팅 시스템** 도입 (React Router)
- [ ] **네비게이션 바/사이드바** 컴포넌트 구현
- [ ] **메뉴 구조** 상태 관리 (Zustand)
...
**목표**: 단순 위젯 대시보드에서 다기능 애플리케이션으로 확장
...
**마지막 업데이트**: 2025-12-05 (Phase 7.1, 7.2 완료)
...
- GitHub GraphQL API 통합 (커밋, PR, Issue)
- REST API를 통한 실제 커밋 개수 조회
- 지수 백오프 재시도 로직 및 캐싱 메커니즘
- GitHub 위젯 UI 구현 및 설정 모달
- 단위 테스트 및 접근성 테스트 작성
...
- 기술 뉴스 위젯 구현 (Dev.to API 통합)
- Stack Overflow 위젯 구현 (Stack Exchange API 통합)
- 공통 컴포넌트 재사용 (Widget, Skeleton, ErrorState, Modal)
- 설정 관리 (키워드/태그 추가/삭제)
- 캐싱 및 재시도 로직 적용

### Phase 6 완료 (2025-12-05)
- react-grid-layout 통합 (ResponsiveGridLayout)
- 드래그 앤 드롭 및 리사이즈 기능 구현
- 반응형 레이아웃 (lg, md, sm, xs breakpoint)
- 레이아웃 저장 및 복원 (Zustand persist)
- 모든 위젯 대시보드 통합
- 레이아웃 자동 초기화 기능

### Phase 7.1, 7.2 완료 (2025-12-05)
- react-hot-toast 통합 및 알림 시스템 구현
  - 할 일 완료/추가 알림
  - 레이아웃 저장 알림
  - API 호출 실패 알림 (GitHub Rate Limit 포함)
- 접근성 개선
  - 키보드 네비게이션 (Tab 키 순환, 스킵 링크)
  - ARIA 레이블 추가 (role, aria-label, aria-labelledby)
  - 포커스 스타일 개선 (focus-visible)
  - 모달 내 포커스 트랩 구현
  - 명암 대비 WCAG 2.1 AA 기준 준수
